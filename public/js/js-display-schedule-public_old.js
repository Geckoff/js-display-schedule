...
(function( $ ) {
	...
	function Schedule(scheduleConfig, elem) {
		...
		/**
		 * determine if timestamp matches odd date
		 *
		 * @param    int    timestamp      
		 */
		this.isOddDate = function(timestamp) {
			var dateObj = new Date(timestamp * 1000);
			return dateObj.getDate() % 2;
		}
		...
		this.isDayToShowEOInterval = function(startDate, periodOfTime) {
			...
			if (timeDiff > 0) {
				show = !isOdd(timeDiff, periodOfTime);
			}	
			return show;
		}	
		 ...
		 */	
		this.isOdd = function(timeDiff, seconds) {
